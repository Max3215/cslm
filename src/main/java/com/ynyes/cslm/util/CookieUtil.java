...
public class CookieUtil {
	...
	@Autowired
	private static TdUserService tdUserService;
	
   //保存cookie时的cookieName
   private final static String cookieDomainName = "cslm";
   //加密cookie时的网站自定码

   private final static String webKey = "123456";
   //设置cookie有效期是两个星期，根据需要自定义
   private final static long cookieMaxAge = 60 * 60 * 24 * 10;
   //保存Cookie到客户端-------------------------------------------------------------------------
   //在CheckLogonServlet.java中被调用
   //传递进来的user对象中封装了在登陆时填写的用户名与密码

   public static void saveCookie(TdUser user, HttpServletResponse response) {
          //cookie的有效期

...
<#list user.shippingAddressList as address>
    ...
    <section class="address_list">
        <div class="tit">
          ...
          <a href="/touch/user/address/update?id=${address.id?c}&pointId=${goods.id?c}"></a>
          <a href="/touch/user/address/delete?id=${address.id?c}&pointId=${goods.id?c}"></a>
        </div>
...
<p class="add_btn_box"><a href="/touch/user/address/update?pointId=${goods.id?c}" class="add_btn">添加收货地址</a></p>

...
<body>
  ...
  <section class="collection_list">
    <ul>
    <#if distributorList??>
        <#list distributorList.content as cg>     
      <li>
        <i></i>
        <a href="/touch/index?id=${cg.id?c}" class="pic"><img src="${cg.imageUri!''}" /></a>
        <a href="/touch/index?id=${cg.id?c}" class="name">${cg.title!''}</a>
        <menu>
        <#--
        <a href="/touch/user/collect/delshop?id=${cg.id?c!''}" class="btn">取消收藏</a>
        -->
        <a href="/touch/index?id=${cg.id?c}" class="btn btn01">进入店铺</a>
        </menu>
        <div class="clear"></div>
      </li>
        </#list>
     <#else>
    <div>
        <p style="text-align:center">没有找到您要搜索的店铺哦！</p>
    </div>
    </#if>
    </ul>
  </section>

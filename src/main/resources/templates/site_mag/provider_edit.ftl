...
<div class="tab-content" style="display: block;">
    ...
    <dl>
        <dt>登录名</dt>
        <dd>
            <input name="username" type="text" value="<#if provider??>${provider.username!""}</#if>" class="input normal" ajaxurl="/Verwalter/provider/check<#if provider??>?id=${provider.id?c}</#if>" datatype="s" sucmsg=" ">
    ...
    <dl>
        <dt>虚拟账号</dt>
        <dd>
            <input name="virtualAccount" type="text" value="<#if provider??>${provider.virtualAccount!""}</#if>" class="input normal"  datatype="*6-20" sucmsg=" ">

...
<div class="mymember_main">
  ...
  <div class="mymember_center add_width">
    <div class="mymember_info mymember_info01 no_width">
      ...
      <table>
        <tr>
          <th width="150" rowspan="2"><a class="mymember_header p_left"><img src="${supply.imageUri!'/client/user_img.png'}" height="120px;" width="120px;"/></a></th>
          <td>账户名称：${supply.title!''}</td>
          <td><a href="#" class="btn">修改</a></td>
        </tr>
        <tr>
          <td>账户余额：¥${supply.virtualMoney?string('0.00')}</td>
          <td><a href="/supply/topup1" class="btn">充值</a></td>
          <td><a href="/supply/draw1" class="btn">提现</a></td>
        </tr>
      </table>
    </div>
            ...
            <#if pay_record_page.totalPages gt 0>
                <#list 1..pay_record_page.totalPages as page>
                    <#if page <= 3 || (pay_record_page.totalPages-page) < 3 || (pay_record_page.number+1-page)?abs<3 >
                        ...
                        <#else>
                            <a href="/supply/account?page=${page-1}">${page}</a>
                        </#if>
                        <#assign continueEnter=false>
                    <#else>
                        <#if !continueEnter>
